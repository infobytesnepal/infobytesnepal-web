...
export default async function ServiceInquiriesPage({ searchParams }: Props) {
  ...
  return (
    <div>
      <h1 className="text-3xl font-semibold text-deep-navy">Service Inquiries</h1>
      <AdminCard className="mt-6">
        <form className="grid gap-3 md:grid-cols-[150px_240px_1fr_auto]">
          ...
          <select name="service" defaultValue={params.service || ""} className="rounded-2xl border border-primary-blue/15 px-4 py-3">
          ...
          <input name="q" defaultValue={params.q || ""} placeholder="Search name, organization, phone, email, service" className="rounded-2xl border border-primary-blue/15 px-4 py-3" />
          <SaveButton>Filter</SaveButton>
        </form>
        <form action={markAllServiceInquiriesRead} className="mt-4">
      ...
      <div className="mt-5 grid gap-4">
          ...
          <AdminCard key={item.id}>
            <div className="flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between">
              <div>
                ...
                <p className="mt-3 w-fit rounded-full bg-soft-green px-4 py-2 text-sm font-semibold text-deep-navy">{item.serviceType}</p>
                {item.remarks && <p className="mt-4 leading-7 text-dark-text/75">{item.remarks}</p>}
                <p className="mt-3 text-xs text-dark-text/55">Submitted via services inquiry form</p>
              </div>
              <div className="flex flex-wrap gap-2">
                {!item.isRead && (
                  <form action={markServiceInquiryRead}>
                    <input type="hidden" name="id" value={item.id} />
                    <button className="rounded-full bg-deep-navy px-4 py-2 text-sm font-semibold text-white">Mark as read</button>
                  </form>
                )}
                <form action={deleteServiceInquiry}>
                  <input type="hidden" name="id" value={item.id} />
                  <ConfirmButton message="Delete this service inquiry?" className="rounded-full border border-primary-blue/20 px-4 py-2 text-sm font-semibold text-deep-navy">
                    Delete
                  </ConfirmButton>
                </form>
              </div>
        ...
        {!rows.length && <AdminCard>No service inquiries found.</AdminCard>}
      </div>
    </div>
  );
}
